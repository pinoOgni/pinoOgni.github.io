...
$('.filtr-wrapper').magnificPopup({type:'image',delegate:'.image-pop',gallery:{enabled:true},zoom:{enabled:true,duration:300,easing:'ease-in-out'}});$(".owl-carousel").owlCarousel({items:1,loop:true,dots:true,autoplay:true,autoplayTimeout:3000});var vh=$(window).height();if(navigator.userAgent.indexOf('Opera Mini')!=-1){work.start();happyClient.start();projects.start();courses.start();coffee.start();$('.skillbar').each(function(){$(this).find('.skillbar-bar').animate({width:jQuery(this).attr('data-percent')},0);});$("input").removeClass("form-control");$("input").css({"width":"100%","height":"50px","background":"#fff"});$(".navigation-icon").css("display","none");}
if(navigator.userAgent.indexOf('UCBrowser')!=-1){$(".navigation-icon").css("display","none");$(".fun-facts").css({"display":"table","margin":"auto"});work.start();happyClient.start();projects.start();courses.start();coffee.start();$('.skillbar').each(function(){$(this).find('.skillbar-bar').animate({width:jQuery(this).attr('data-percent')},0);});}
